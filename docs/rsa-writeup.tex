...
An IBM study in the 1990s concluded that approximately one random bit flip will occur per month, per 256 MB of RAM \cite{cosmic}. The probability of a random bit flip of one byte is thus $1 / 256000 / (30 * 24 * 60 * 60) \approx 4.519 * 10^{-11}$ per second. When generating a new key, we typically spend ~20 seconds running consecutive Miller-Rabin tests, on average. Our Miller-Rabin test runs on 512-bit integers. Thus, the probability of some random bit flip occuring on the input to our Miller-Rabin test while it runs is $4.627 * 10^{-7} * 20 * 512 \approx 4.628 * 10^{-7}$.
